...
# Based on
# https://github.com/elasticsearch/logstash-contrib/blob/master/lib/logstash/outputs/influxdb.rb
#
...
class LogStash::Outputs::InfluxDB < LogStash::Outputs::Base
  ...
  config :db, :validate => :string, :default => "logstash"
  ...
  config :series, :validate => :string, :default => "metrics"
    ...
    @coerce_values.each do |column, value_type|
      if sprintf_points.has_key?(column)
        begin
          ...
          when "array"
            @logger.debug("Converting column #{column} to type #{value_type}: Current value: #{sprintf_points[column]}")
            sprintf_points[column] = sprintf_points[column].to_s
          else
